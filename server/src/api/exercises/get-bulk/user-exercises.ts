...
router.get(
  ...
  async (req: any, res: any, next: any) => {
    ...
    if (!exUids || exUids.length < 1)
      return res.status(400).send('Request is empty.');

    //check if a single exUid came through
    if (typeof exUids === 'string') {
      if (!mongoose.Types.ObjectId.isValid(exUids))
        return res.status(400).send('Invalid id associated with request');
    } else {
      for (let i = 0; i < exUids.length; i++) {
        if (!mongoose.Types.ObjectId.isValid(exUids[i]))
          return res.status(400).send('Invalid id associated with request');
      }
    }
