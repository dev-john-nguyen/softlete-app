...
import {
  fetchAllAthleteExercises,
  fetchAthleteHealthData,
  fetchAthleteWorkouts,
  getAthleteFriends,
  sendFriendRequest,
} from '../../services/athletes/actions';
import Loading from '../../components/elements/Loading';
import {
  SET_ATHLETE_VIEW_WORKOUT,
  STORE_ATHLETE_EXERCISES,
...
const AthleteDashboard = ({
      ...
      headerRight: () => (
        <View
          ...
          <Pressable
            ...
            onPress={() =>
              navigation.navigate(NetworkStackScreens.AthleteModal)
            }
            hitSlop={5}>
  ...
  const onMessage = async () => {
    const { athlete }: { athlete: AthleteProfileProps } = route.params;
    navigation.navigate(NetworkStackScreens.Message, {
  ...
  const onFetchMonths = async (add?: boolean) => {
    ...
    if (mount.current) {
      setMonthsFetched(prev =>
        [...prev, { month, year }].sort(
          (a, b) =>
  ...
  const navigateToWo = async (workoutUid: string) => {
    const workout = workouts.find(w => w._id === workoutUid);
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    fetchAthleteWorkouts: (uid: string, fromDate: string, toDate: string) =>
      dispatch(fetchAthleteWorkouts(uid, fromDate, toDate)),
    ...
    getCurChat: (userUid: string, chatId: string) =>
      dispatch(getCurChat(userUid, chatId)),
    fetchAllAthleteExercises: (uid: string) =>
      dispatch(fetchAllAthleteExercises(uid)),
    fetchAthleteHealthData: (uid: string) =>
      dispatch(fetchAthleteHealthData(uid)),
