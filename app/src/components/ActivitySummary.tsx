import React from 'react';
import { FlexBox } from '@app/ui';
import { ScrollView } from 'react-native';
import {
  PrimaryText,
  InfoListBox,
  LineChartGraph,
  formatNumber,
} from './elements';
import { Colors } from '@app/utils';
import WorkoutTracker from 'src/classes/WorkoutTracker';

type Props = {
  workout: WorkoutTracker;
  hideDate?: boolean;
  applyPadding?: boolean;
  iconColor?: string;
};

const ActivitySummary: React.FC<Props> = ({
  workout,
  hideDate,
  applyPadding = true,
  iconColor = Colors.white,
}) => {
  const data = workout.getFormattedData();
  const segments = workout.getSegmentData();

  return (
    <FlexBox column>
      {!hideDate && (
        <PrimaryText marginBottom={10} paddingRight={15} paddingLeft={15}>
          Date: {workout.getDate()}
        </PrimaryText>
      )}
      <ScrollView
        contentContainerStyle={{ paddingBottom: 50 }}
        showsVerticalScrollIndicator={false}>
        <FlexBox
          column
          paddingRight={applyPadding ? 15 : 0}
          paddingLeft={applyPadding ? 15 : 0}>
          <InfoListBox
            secondary
            icon="fire"
            desc={data?.calories || ''}
            color={iconColor}
            label="Calories Burned"
            screenWidthPct={0.8}
            marginBottom={10}
          />

          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <InfoListBox
              secondary
              icon="clock"
              desc={data?.duration || ''}
              color={iconColor}
              label="Time"
            />

            <InfoListBox
              secondary
              icon="ruler"
              desc={data?.distance || ''}
              color={iconColor}
              label="Distance"
            />

            <InfoListBox
              secondary
              icon="run"
              desc={data?.averagePace || ''}
              color={iconColor}
              label="Avg Pace"
            />

            <InfoListBox
              secondary
              icon="pyramid"
              desc={data?.averageAltitude || ''}
              color={iconColor}
              label="Avg Altitude"
            />
          </ScrollView>

          <InfoListBox
            secondary
            icon="heart"
            desc={data?.averageHeartRate || ''}
            color={iconColor}
            label="Average Heart Rate"
            screenWidthPct={0.5}
            marginTop={10}
            marginBottom={10}
          />
        </FlexBox>
        <FlexBox marginTop={10} width="100%">
          <LineChartGraph
            header="Heart Rate Trend"
            subHeader="bpm"
            data={data?.heartRates || []}
            decimalPlaces={2}
            paddingRight={35}
            formatYLabel={numStr => formatNumber(numStr)}
            applyHeaderPadding
          />
        </FlexBox>

        <FlexBox column paddingRight={15} paddingLeft={15} marginTop={5}>
          <PrimaryText marginBottom={5} size="medium" bold>
            Segments
          </PrimaryText>
          {segments.length < 1 ? (
            <FlexBox>
              <PrimaryText>No Segments Found</PrimaryText>
            </FlexBox>
          ) : (
            segments.map((segment, i) => (
              <FlexBox column key={i} marginBottom={5}>
                <PrimaryText opacity={0.6} marginBottom={2}>
                  Mile {segment.order}:
                </PrimaryText>
                <PrimaryText opacity={1} marginBottom={2}>
                  Pace: {segment.paceLabel}
                </PrimaryText>
                <PrimaryText opacity={1}>
                  Altitude: {segment.averageAltitudeLabel}
                </PrimaryText>
              </FlexBox>
            ))
          )}
        </FlexBox>
        <FlexBox marginTop={10} marginBottom={30} column>
          <LineChartGraph
            header="Pace Trend"
            subHeader="Minute/Mile"
            data={workout.getPacesInTimeDecimal()}
            decimalPlaces={2}
            paddingRight={35}
            formatYLabel={numStr => formatNumber(numStr)}
            applyHeaderPadding
          />
          <LineChartGraph
            header="Altitude Trend"
            subHeader="Meters"
            data={workout.getAltitudes()}
            paddingRight={35}
            formatYLabel={numStr => formatNumber(numStr)}
            applyHeaderPadding
          />
        </FlexBox>
      </ScrollView>
    </FlexBox>
  );
};

export default ActivitySummary;
