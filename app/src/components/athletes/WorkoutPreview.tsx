...
const AthletePreviewItem = ({
  ...
  const isLiked = () =>
    likedWoIds.find(id => id === workout._id) ||
    workout.likeUids?.find(id => id === uid);
  ...
  return (
    <View style={{ marginBottom: 5 }}>
      <Pressable onPress={() => onPress(workout._id)} style={styles.container}>
        {!!workout.imageUri && (
          <FastImage style={styles.image} source={{ uri: workout.imageUri }} />
        )}
        <View style={styles.content}>
          <View style={styles.headerContainer}>
            <SecondaryText
              styles={[styles.name, { color: renderColor(workout.status) }]}
              numberOfLines={1}
              bold>
              {workout.name}
            </SecondaryText>
