      ...
      switch (friend) {
        case 'add':
          return (
            <ProfileButton
              text="Follow"
              onPress={() => sendRequest && sendRequest(FriendStatus.pending)}
            />
          );
        case 'pending':
          return (
            <ProfileButton
              text="Pending"
              onPress={() => sendRequest && sendRequest(FriendStatus.denied)}
            />
          );
        case 'respond':
          return (
            <View
              ...
              <ProfileButton
                text="Accept"
                onPress={() =>
                  sendRequest && sendRequest(FriendStatus.accepted)
                }
              />
              <ProfileButton
                text="Decline"
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <View style={{ flex: 1, marginLeft: StyleConstants.smallMargin }}>
          ...
          <SecondaryText styles={styles.athlete}>
            ({user.athlete})
          </SecondaryText>
        </View>
      </View>
