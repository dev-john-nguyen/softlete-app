...
export const saveOfflineData =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (!diff.isEmpty()) {
      ...
      if (workoutHeadersStore.length > 0)
        await updateOfflineWoHeaders(workoutHeadersStore, callback);
      //save added new workouts and remove workouts deleted workouts
      if (workoutsUpdateStore.length > 0)
        await saveOfflineWos(workoutsUpdateStore, callback, getState);
      //update health data of the workouts that changed health data
      if (updatedWoHealthData.length > 0)
        await updateWoDataBatch(updatedWoHealthData);
...
const updateWoDataBatch = async (healthData: HealthDataProps[]) => {
  return request('POST', PATHS.workouts.batch.healthData, requestCallback, {
    healthData,
  });
};

const updateOfflineWoHeaders = async (
  headers: WorkoutHeaderProps[],
  callback: (msg: string) => void,
) => {
  if (headers.length < 1) return;
  callback('updating workout headers...');
  await request('POST', PATHS.workouts.updateHeader, requestCallback, {
    workouts: headers,
  }).catch(err => {
    console.log(err);
...
const updateOfflineWos = async (
  newWorkout: WorkoutProps,
  prevWorkout: WorkoutProps,
  ...
  if (newWorkout.status !== prevWorkout.status) {
    if (newWorkout.status === WorkoutStatus.completed) {
      const strainRating = newWorkout.strainRating
        ? newWorkout.strainRating
        : 0;
      const reflection = newWorkout.reflection ? newWorkout.reflection : '';
      const imageProps = {
        uri: newWorkout.localImageUri,
        base64: newWorkout.imageBase64,
      };
...
const saveOfflineWos = async (
  ...
  const wosRemoved = workouts
    .filter(wo => wo._id.length > 10)
    .map(wo => ({ _id: wo._id, imageUri: wo.imageUri }));
  ...
  if (wosRemoved.length > 0) {
    ...
    await request('POST', PATHS.workouts.remove, requestCallback, {
      workoutUids: wosString,
    });
  }
  ...
  if (newWorkouts.length > 0) {
    ...
    const { data: savedWos }: { data?: NewWorkoutProps[] } = await request(
      'POST',
      PATHS.workouts.create,
      requestCallback,
      { workouts: workoutHeaders },
    );

    if (!savedWos || savedWos.length !== newWorkouts.length)
      return callback('Failed to save new workouts...');
    ...
    const workoutExercises = newWorkouts
    ...
    //save health data
    const workoutHealthData: any = newWorkouts
      .filter(w => w.healthData)
      .map((wo, i) => {
        return {
          ...wo.healthData,
    ...
    if (workoutHealthData.length > 0)
      await updateWoDataBatch(workoutHealthData);
    ...
    for (let i = 0; i < workoutHeaders.length; i++) {
      ...
      if (
        ...
        const path = await processImage(
          lsWo.imageBase64,
          genWoImagePath(uid, lsWo.imageId),
        )(requestCallback);
        if (path && wo._id) {
          woImages.push({
            _id: wo._id,
    ...
    if (woImages.length > 0) {
      await request('POST', PATHS.workouts.batch.images, requestCallback, {
        workoutsImages: woImages,
      }).catch(err => {
        console.log(err);
