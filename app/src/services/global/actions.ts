  ...
  () => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    try {
      await AsyncStorage.multiSet([
        [LocalStoragePaths.lastFetchedWos, JSON.stringify(workouts)],
        [LocalStoragePaths.offline, '1'],
      ]);
    } catch (err) {
      dispatch(
        setBanner(BannerTypes.error, 'Failed to go offline. Please try again'),
      );
    }
...
export const goOnline =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const lastFetchedWos = await AsyncStorage.getItem(
...
export const getGlobalVars =
  () => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    request('GET', PATHS.global.get, dispatch).then(({ data }) => {
      if (data) {
        dispatch({
...
export const processBatches =
  () => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const batch = await AsyncStorage.multiGet([
    ...
    try {
      ...
      if (strBatchItems) {
        const batchItems = JSON.parse(
          strBatchItems,
        ) as ExercisesVideoBatchProps[];
  ...
  (dispatch: AppDispatch, getState: () => ReducerProps) => {
    if (batch.length > 0) {
      ...
      for (let i = 0; i < batch.length; i++) {
        const item = batch[i];
        //I don't have to await I can compress multiple at a time
        processImage(item.base64, item.imageId)(dispatch, getState).catch(err =>
          console.log(err),
        );
      }
    }
