...
import {
  calcAnalytics,
  calcAnalyticsData,
  handleAnalyticsFetched,
} from './utils';
import { fetchExercises } from '../exercises/actions';
  ...
  (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    workout.exercises.forEach(e => {
      if (e.exerciseUid) {
        ...
        const foundIndex = analyticsStore.findIndex(
          a => a.exerciseUid === e.exerciseUid,
        );

        if (foundIndex > -1) {
          //check if data already exists by looping through data and determine if _id unique
          const wOExIndex = analyticsStore[foundIndex].data.findIndex(
            dta => dta.workoutExerciseUid === e._id,
          );
    ...
    const newAnalyticsStore = analyticsStore.map(a => {
      const aIndex = fetchedAnalytics.findIndex(
...
export const fetchPinExerciseAnalytics =
  (
    fromDate: string,
    toDate: string,
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const fetchedAnalytics = await fetchExerciseAnalytics(
      fromDate,
      toDate,
      pinExercises.map(p => p.exerciseUid),
      athlete,
    )(dispatch, getState).then(res => (res ? res : []));
    ...
    for (let i = 0; i < emptyPins.length; i++) {
      if (!emptyPins[i].exercise) {
        const inStore = exercisesStore.find(
          e => e._id === emptyPins[i].exerciseUid,
        );
        if (inStore) {
...
export const fetchExerciseAnalytics =
  (
    fromDate: string,
    toDate: string,
    exerciseUids: string[],
...
export const fetchExerciseAnalyticsDates =
  (dates: string[], exerciseUids: string[], athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    let uid = '';
