...
export const fetchAllUserExercises =
...
export const createNewExercise =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (
      ...
      exerciseProps.localUrl &&
      exerciseProps.localThumbnail
    ) {
      saveVideos(
        exerciseProps.localUrl,
        exerciseProps.videoId,
...
export const searchExercises =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const queryExercise = exercises.data.filter(
      e => e.name && regex.test(e.name.toLowerCase()),
    );
...
export const searchByCat =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const queryExercise = exercises.data.filter(e =>
...
export const updateExercise =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (
      exerciseProps.videoId &&
      exerciseProps.videoId !== exerciseStore.videoId
    ) {
      //video changed
      ...
      if (exerciseProps.localUrl && exerciseProps.localThumbnail) {
        saveVideos(
          ...
          exerciseProps.videoId,
          exerciseProps.localThumbnail,
        )(dispatch, getState).catch(err => console.log(err));
      }
    }
...
export const removeExercise =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (targetEx.videoId) {
      removeVideo(user.uid, targetEx.videoId)(dispatch, getState).catch(err =>
        console.log(err),
      );
    }
...
export const fetchExercises =
  (exerciseUids: string[], athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    let uid = '';
    ...
    if (global.offline) {
      ...
      const lsExercisesStr = await AsyncStorage.getItem(
        LocalStoragePaths.exercises,
      );
      if (lsExercisesStr) {
        const localStoreExs = JSON.parse(lsExercisesStr) as ExerciseProps[];
        const filterExs = localStoreExs.filter(e =>
          exerciseUids.find(id => id === e._id),
        );
        return filterExs;
      }
      return [];
    ...
    const res = await request(
      'GET',
      PATHS.exercises.fetchBulkUsers(exerciseUids, uid),
      dispatch,
    );
...
export const insertExercisesIntoWorkouts =
  (workouts: Omit<WorkoutProps, 'date'>[], athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    for (let i = 0; i < workouts.length; i++) {
      if (workouts[i].exercises?.length > 0) {
        workouts[i].exercises = await insertExercises(
...
export const insertExercises =
  (exercises: WorkoutExerciseProps[], athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    //this function updates the workout with extra data in storage or if not will fetch it from server
    ...
    if (exerciseUidsToFetch.length > 0) {
      //remove duplicates
      const exercisesFetched = await fetchExercises(
        _.uniq(exerciseUidsToFetch),
        athlete,
      )(dispatch, getState);
...
export const updateAnalyticsExercises =
  (analytics: AnalyticsProps[], athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
...
export const updatePinExercisesWithExercises =
  (pins: PinExerciseProps[]) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const exercisesStore = getState().exercises.data;
...
export const fetchMusclesAndEquipments =
  () => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
...
export const fetchLocalStoreExercisesToState =
