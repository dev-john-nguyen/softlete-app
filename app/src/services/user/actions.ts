...
export const login = (token: string) => async (dispatch: AppDispatch) => {
  ...
  const { data, networkError }: { data?: UserProps; networkError?: boolean } =
    await request('GET', PATHS.signin.login, dispatch);
...
export const updateProfile =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (base64) {
      ...
      savedImageUri = await processImage(
        base64,
        `${uid}/profile/img.jpg`,
      )(dispatch);
...
export const updatePinExercises =
  (pinProps: PinExerciseProps, pin: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    //max length is 10
    ...
    if (pinExercises.length >= Limits.maxPinExs && pin) {
      dispatch(
        setBanner(
          BannerTypes.warning,
          'Exceeded the max limit for pinned exercises.',
        ),
      );
      return;
    }
