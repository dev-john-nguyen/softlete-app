...
export interface AthleteActionProps {
  fetchAthleteWorkouts: (
    ...
    fromDate: string,
    toDate: string,
  ) => Promise<WorkoutProps[] | undefined>;
  setAthleteViewWorkout: (
    workoutUid: string,
    isProgram?: boolean,
  ) => Promise<void>;
  sendFriendRequest: (
    userUid: string,
    sendFriendRequest: FriendStatus,
  ) => Promise<void>;
  fetchAthleteProfiles: (
    userUids: string[],
  ) => Promise<void | AthleteProfileProps[]>;
  ...
  fetchAllAthleteExercises: (
    athleteUid: string,
  ) => Promise<void | undefined | ExerciseProps[]>;
  fetchAthleteHealthData: (
    athleteUid: string,
  ) => Promise<void | HealthDataProps[] | undefined>;
