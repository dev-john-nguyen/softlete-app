...
export const fetchAllAthleteExercises =
  (athleteUid: string) => async (dispatch: AppDispatch) => {
    return request<ExerciseProps[]>(
      'GET',
      PATHS.exercises.fetchAllUsers(athleteUid),
      dispatch,
    )
      .then(({ data }) => {
        return data;
...
export const fetchAthleteWorkouts =
  (athleteUid: string, fromDate: string, toDate: string) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const { data }: { data?: WorkoutProps[] } = await request<WorkoutProps[]>(
      'GET',
      PATHS.workouts.fetch(fromDate, toDate, athleteUid),
      dispatch,
    ).catch(err => {
      console.log(err);
      return {};
    ...
    if (data) {
      if (data.length < 1) {
        ...
        const workouts = await insertExercisesIntoWorkouts(data, true)(
          dispatch,
          getState,
        );
...
export const setAthleteViewWorkout =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (workout && workout.exercises) {
      workout.exercises = await insertExercises(workout.exercises, true)(
        dispatch,
        getState,
      );
...
export const sendFriendRequest =
  (userUid: string, status: FriendStatus) => async (dispatch: AppDispatch) => {
    return request<FriendProps[]>('POST', PATHS.friends.update, dispatch, {
      userUid,
      status,
    })
      .then(({ data }) => {
...
export const fetchAthleteProfiles =
  (userUids: string[]) => async (dispatch: AppDispatch) => {
    return request<AthleteProfileProps[]>(
      'GET',
      ATHLETE_PATHS.getBulk(userUids),
      dispatch,
    )
      .then(({ data }) => {
        if (data) {
...
export const getAthletesProfile =
  (userUids: string[]) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
...
export const likeWorkout =
  (workoutUid: string) => async (dispatch: AppDispatch) => {
...
export const fetchAthleteHealthData =
  (athleteUid: string) => async (dispatch: AppDispatch) => {
    return request<HealthDataProps[]>(
      'GET',
      PATHS.workouts.getAllHealthData(athleteUid),
      dispatch,
    )
      .then(({ data }) => {
        return data;
...
export const getAthleteFriends =
  (athleteUid: string) => async (dispatch: AppDispatch) => {
    return request<FriendProps[]>(
      'GET',
      PATHS.friends.getFriends(athleteUid),
      dispatch,
    )
      .then(({ data }) => {
        if (data) {
          ...
          uidsToFetch.length > 0 &&
            fetchAthleteProfiles(uidsToFetch)(dispatch).catch(err =>
              console.log(err),
            );
