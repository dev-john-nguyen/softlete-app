...
import {
  ...
  SET_PROGRAMS,
  SET_TARGET_PROGRAM,
  REMOVE_PROGRAM_WORKOUT,
  REMOVE_PROGRAM,
  UPDATE_PROGRAM_HEADER,
  SET_GENERATED_PROGRAMS,
  REMOVE_GENERATED_PROGRAM,
  UPDATE_PROGRAM_WORKOUTS,
  UPDATE_PROGRAM_ACCESS_CODE,
  UPDATE_PROGRAM_WO_HEALTH_DATA,
  SET_PROGRAM_VIEW_WORKOUT,
} from './actionTypes';
...
import {
  ...
  SET_ATHLETE_TARGET_PROGRAM,
  SET_ATHLETE_VIEW_WORKOUT,
...
export const updateProgramHeader =
  (programData: NewProgramProps, imageBase64?: string) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const path = programData._id
      ? PATHS.programs.update
      : PATHS.programs.create;
    ...
    if (imageBase64 && programData._id) {
      //process image
      imageUri = await processImage(
        imageBase64,
        `${uid}/programs/${programData._id}`,
      )(dispatch);
    ...
    request('POST', path, dispatch, programData)
      .then(async ({ data }) => {
        if (data) {
          ...
          dispatch(
            setBanner(BannerTypes.default, 'Program successfully saved!'),
          );
        }
      })
...
export const setTargetProgram =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const { data }: { data?: ProgramWorkoutProps[] } = await request(
      'GET',
      PATHS.programs.getWorkouts(uid, programHeader._id),
      dispatch,
    );
...
export const updateProgramWorkoutExercises =
  (
    workoutUid: string,
    ...
    await request<WorkoutExerciseProps[]>(
      ...
      .then(async ({ data }) => {
        if (data) {
          ...
          const stateWorkout = findAndUpdateWorkoutExercises(
            programWorkouts,
            workoutUid,
            data,
          );
          ...
          stateWorkout.exercises = await insertExercises(
            stateWorkout.exercises,
          )(dispatch, getState);
...
export const setProgramViewWorkout =
  (workoutUid: string, programUid: string, athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const { user, program, athletes } = getState();

    let targetWorkout: ProgramWorkoutProps | undefined;

    const targetProgram = athlete
      ? athletes.targetProgram
      : program.programs.find(p => p._id === programUid);

    if (!targetProgram) {
      dispatch(
        setBanner(BannerTypes.warning, 'Unable to identify the target program'),
      );
    ...
    if (!targetWorkout) {
      dispatch(
        setBanner(BannerTypes.warning, 'Unable to identify the target workout'),
      );
    ...
    if (!targetWorkout.exercises) {
      const { data } = await request(
        'GET',
        PATHS.programs.getWorkoutExercises(user.uid, workoutUid),
        dispatch,
      );

      targetWorkout.exercises = data
        ? [...(data as WorkoutExerciseProps[])]
        : [];
    ...
    athlete
      ? dispatch({ type: SET_ATHLETE_VIEW_WORKOUT, payload: targetWorkout })
      : dispatch({ type: SET_PROGRAM_VIEW_WORKOUT, payload: targetWorkout });
  };

export const duplicateProgramWorkout =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    const { data }: { data?: WorkoutProps } = await request(
      'POST',
      PATHS.programs.duplicateWorkout,
      dispatch,
      { daysFromStart, workoutUid: copiedProgramWorkout },
    );
    ...
    request('POST', PATHS.programs.removeExercise, dispatch, {
      ...
      .then(() => {
        ...
        const updatedWorkout = findAndUpdateWorkoutExercises(
          [viewWorkout],
          viewWorkout._id,
          [{ _id: programWorkoutExercise._id, remove: true }] as any,
        );
...
export const generateProgram =
  (programUid: string, startDate: string, accessCode?: string) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const res = await request('POST', PATHS.programs.generate, dispatch, {
      _id: programUid,
      startDate: startDate,
      accessCode,
    });
    ...
    if (res && res.data) {
      ...
      const { workouts, program } = res.data as {
        workouts: WorkoutProps[];
        program: ProgramProps;
      };

      //populate workouts
      const updatedWorkouts = await insertExercisesIntoWorkouts(workouts)(
        dispatch,
        getState,
      );
...
export const fetchGeneratedPrograms =
  (athlete?: boolean) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    let uid = '';
    ...
    const res = await request(
      'GET',
      PATHS.programs.generated.get(uid),
      dispatch,
    );
...
export const removeGeneratedProgram =
  (programUid: string) => async (dispatch: AppDispatch) => {
    const res = await request(
      'POST',
      PATHS.programs.generated.remove,
      dispatch,
      { programUid },
    );
...
export const updateProgramExerciseData =
  ...
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    return request('POST', PATHS.programs.updateExerciseData, dispatch, {
      dataArr: dataWoPerform,
    })
      .then(res => {
        if (res && res.data) {
...
export const updateProgramWoHealthData =
  (
