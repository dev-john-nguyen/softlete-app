import {
  ...
  INSERT_MY_PROGRAM_LIKE,
  UPDATE_PROGRAM_WO_HEALTH_DATA,
  SET_PROGRAM_VIEW_WORKOUT,
  SET_PROGRAM_WORKOUT_HEADER,
} from './actionTypes';
import {
  findAndUpdateProgram,
  findAndRemoveProgramWorkout,
  findAndRemoveProgram,
  findAndInsertLikeProgram,
  findAndUpdateProgramWoData,
} from './utils';
import {
  findAllAndUpdateExerciseProps,
  findAndUpdateWorkouts,
} from '../workout/utils';
  ...
  switch (action.type) {
    case UPDATE_EXERCISE:
      ...
      if (state.targetProgram && state.targetProgram.workouts) {
        const updatedProgramWorkouts = findAllAndUpdateExerciseProps(
          state.targetProgram.workouts,
          action.payload.exercise as ExerciseProps,
        );
        return {
          ...state,
          targetProgram: {
    ...
    case SET_GENERATED_PROGRAMS:
      return {
        ...state,
        generatedPrograms: findAndUpdateWorkouts(
    ...
    case REMOVE_GENERATED_PROGRAM:
      return {
        ...state,
        generatedPrograms: findAndRemoveProgram(
          state.generatedPrograms,
          action.payload,
        ),
      };
    ...
    case UPDATE_PROGRAM_WORKOUTS:
      newProgram = {
        ...state.targetProgram,
        workouts: findAndUpdateWorkouts(
          state.targetProgram.workouts,
          action.payload as ProgramWorkoutProps[],
        ),
      };
    ...
    case REMOVE_PROGRAM_WORKOUT:
      newProgram = {
        ...state.targetProgram,
        workouts: findAndRemoveProgramWorkout(
          state.targetProgram.workouts,
          action.payload,
        ),
      };
    ...
    case INSERT_MY_PROGRAM_LIKE:
      return {
        ...state,
        programs: findAndInsertLikeProgram(
          state.programs,
          action.payload.programUid,
          action.payload.uid,
        ),
      };
    case UPDATE_PROGRAM_WO_HEALTH_DATA:
      newProgram = {
        ...state.targetProgram,
        workouts: findAndUpdateProgramWoData(
          state.targetProgram.workouts,
          action.payload.workoutUid,
          action.payload.data,
        ),
      };
      return {
