...
export interface ProgramWorkoutHeaderProps
  extends Omit<WorkoutHeaderProps, 'date' | 'programUid'> {
  daysFromStart: number;
  programTemplateUid: string;
  imageUri?: string;
}

export interface ProgramWorkoutProps
  extends Omit<WorkoutProps, 'date' | 'programUid'> {
  daysFromStart: number;
  programTemplateUid: string;
}
...
export interface ProgramActionProps {
  ...
  updateProgramHeader: (
    programData: NewProgramProps,
    imageBase64?: string,
  ) => Promise<void | undefined>;
  fetchPrograms: (athlete?: boolean) => Promise<void>;
  setTargetProgram: (
    programHeader: ProgramHeaderProps,
    athlete?: boolean,
    softlete?: boolean,
  ) => Promise<void | ProgramProps>;
