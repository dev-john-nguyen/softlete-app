...
import {
  genExerciseVideoPath,
  genExerciseVideoThumbnailPath,
} from '../../utils/MediaPaths';

const saveVideos =
  (localUrl: string, videoId: string, localThumbnail: string) =>
  async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    //first compress
    const { user } = getState();
    let exercisesVideoBatch = _.cloneDeep(
      getState().global.exercisesVideoBatch,
    );

    let batch: ExercisesVideoBatchProps | undefined = exercisesVideoBatch.find(
      e => e.videoId === videoId,
    );
    ...
    removed = await updateExerciseVideoBatch(batch, dispatch, getState).catch(
      err => console.log(err),
    );
    if (removed) return;
    ...
    try {
      if (!uri) {
        batch.url = await saveVideoToStorage(
          genExerciseVideoPath(user.uid, videoId),
          batch.compressedUrl,
        );
        //create thumbnail
        ...
        if (thumbnailBase64) {
          //save thumbnail
          batch.thumbnail = await saveImage(
            thumbnailBase64,
            genExerciseVideoThumbnailPath(user.uid, videoId),
          )();
        }
      }
    } catch (err: any) {
    ...
    const { data } = await request(
      'POST',
      PATHS.video.upload,
      (({ payload }: any) => console.log(payload.msg)) as any,
      { videoId: videoId, url: batch.url, thumbnail: batch.thumbnail },
    );
...
const notifyError = async (
  ...
  const exercises = _.cloneDeep(getState().exercises);
...
const updateExerciseVideoBatch = async (
  ...
  if (removeItem) {
    const findIndex = exercisesVideoBatch.findIndex(
      item => item.videoId == batch.videoId,
    );
    if (findIndex > -1) exercisesVideoBatch.splice(findIndex, 1);
    newBatchStore = [...exercisesVideoBatch];
  } else {
  ...
  AsyncStorage.setItem(
    LocalStoragePaths.exercisesVideosBatch,
